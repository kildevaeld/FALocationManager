...
func _boundingBox (location: CLLocationCoordinate2D, distance: Double, unit: Unit) -> BoundingBox {
    let minLat = locationFromDistance(location.latitude, location.longitude, 0, distance, .Kilometres).latitude
    let maxLat = locationFromDistance(location.latitude, location.longitude, 180, distance, .Kilometres).latitude
    
    let minLng = locationFromDistance(location.latitude, location.longitude, 90, distance, .Kilometres).longitude
    let maxLng = locationFromDistance(location.latitude, location.longitude, 270, distance, .Kilometres).longitude
...
extension NSPredicate {
    ...
    static public func boundingBox(location:CLLocationCoordinate2D, distance: CLLocationDistance, latitudeKeyPath:String, longitudeKeyPath: String) -> NSPredicate {
        ...
        let box = _boundingBox(location, distance, .Kilometres)
